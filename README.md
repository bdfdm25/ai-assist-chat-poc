...
graph TB
    subgraph Browser["🌐 Client (Browser)"]
        subgraph Angular["Angular App :4200"]
            Chat[ChatComponent<br/>Container]
            Messages[MessageList/Input/Item<br/>Presenters]
            ChatSvc[ChatService<br/>Facade]
            WS[WebSocketService<br/>Communication]
    ...
    subgraph Backend["🖥️ NestJS API :3000"]
        subgraph Gateway["WebSocket Layer"]
            WSGateway[WebSocket Gateway<br/>Real-time Streaming]
        end

        subgraph Services["Business Logic"]
            ChatService[Chat Service<br/>Orchestration]
            AIService[AI Service<br/>OpenAI Integration]
    ...
    WS -->|WebSocket<br/>Socket.IO| WSGateway
    WS -->|HTTP/REST| Backend
    CircuitBreaker -->|HTTPS| OpenAI

    style Browser fill:#e1f5ff
    style Backend fill:#fff4e6
    style External fill:#f3e5f5
    style Chat fill:#4CAF50,color:#fff
    style ChatSvc fill:#2196F3,color:#fff
    style WS fill:#FF9800,color:#fff
    style WSGateway fill:#9C27B0,color:#fff
    style AIService fill:#F44336,color:#fff
    style CircuitBreaker fill:#FF5722,color:#fff
```
